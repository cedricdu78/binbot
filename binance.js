...
(async () => {
    while (1) {
        try {
            ...
            await binance.openOrders(undefined, (error, currencies_open) => {
                binance.balance( (error, balances) => {

                    let counter = 0;

                    Object.entries(curr).forEach(function([key, value], index, array){
                        let moy = []
                        binance.candlesticks(value.symbol, interval, (error, res) => {
                            ...
                            let info = (curr.filter(val => val.symbol === value.symbol))[0]['filters']
