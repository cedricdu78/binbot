module.exports = {
    ...
    order: function (currency, volume, stopLoss, stopPrice, openValue, nowValue, timestamp, plusValue, orderId = 0) {
        const order = Object.create(null)
        order.currency = currency
        order.volume = Number(volume)
        order.openPrice = Number(openValue)
        order.nowPrice = Number(nowValue)
        order.stopLoss = Number(stopLoss)
        order.closePrice = Number(stopPrice)
